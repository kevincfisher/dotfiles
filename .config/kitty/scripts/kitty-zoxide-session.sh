...
print_menu_lines() {
  local query-"${1:-}"
  zoxide query -l 2>/dev/null | awk -v OFS='\t'-v color="${base_color}" -v reset="${reset_color}" '{
    path=$0
    n=split(path, parts, "/")
    base=parts[n]
    if (base =="") base=path
...
print_ssh_menu_lines() {
  ...
  local host=""
  local label=""
  local debug_log="/tmp/kitty-zoxide-debug.log"
  ...
  #Extract only concrete host names
  while IFS= read -r host; do
    [[ -z "$host" ]] && continue
    label="ssh-${host}"
...
fzf_out="$(
  fzf --exact --ansi --height=20 --reverse \
    ...
    --prompt="Create new Kitty Session (zoxide + zsh) > " \
    --no-multi \
    --with-nth=2.. \
    --no-sort \
    --tiebreak=index \
