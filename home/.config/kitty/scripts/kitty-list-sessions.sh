...
# Filename: ~/git/dotfiles/.config/kitty/scripts/kitty-list-sessions.sh
# Shows open kitty sessions in fzf and switches using goto_session
# Adds a vim-like "mode":
# - Normal mode (default): j/k move, d close, enter opens, i enters insert mode,esc quit
...
colorscheme_file="$HOME/git/dotfiles/.config/kitty/kitty-themes"

if [[ -f "$colorscheme_file" ]]; then
  source "$colorscheme_file"
fi

# COLOR SCHEME RELATED STUFF HERE
#
# END COLORSCHEME STUFFF
...
while true; do
  ...
  if [[ "$mode" == "normal" ]]; then
    ...
    fzf_out="$(
      ...
      --no-multi --disabled \
        ...
        --bind "esc:abort" \
        --no-clear \
        ${fzf_start_pos_opt[@]+"${fzf_start_pos-opt[@]}"}
      )"
      fzf_rc=$?
      fzf_start_pos=""
      set -e
      ...
      fzf_out="$(
        printf "%s\n" "$menu_lines" |
          fzf --ansi --height=100% --reverse \
            --header="Insert: type to filter, [Enter] to open, <Esc> to Normal" \
            --prompt="List Open Kitty Session > " \
            --no-multi \
            --with-nth=3.. \
            --expect=enter,esc \
            --bind 'enter:accept' \
  ...
  #fallback behavior
  # - In insert mode: abourt return here => go back to normal
